...
def return_ranking_features(features_names, X_train, y_train):
    """
    return 
    ...
    permutation_importance_fit = permutation_importance(rfe, X_test, y_test,
        n_repeats=20,random_state=0)
    print("features permutation importance")
    for name, features_importance in zip(features_names,permutation_importance_fit.importances_mean):
...
def gridsearch_best_model():
    ...
    # based on https://github.com/yuxiaohuang/teaching/blob/main/machine_learning_I/fall_2020/code/p2_shallow_learning/p2_c2_supervised_learning/p2_c2_s5_tree_based_models/code_example/code_example.ipynb
    ...
    for name, clf in pipeline_with_multiple_classifier.items():
        operation_steps[name] = Pipeline([('StandardScaler', StandardScaler()), ('clf', clf)])

    parameters_experiment = {}

    params = [{'clf__multi_class': ['ovr'], 
    ...
    parameters_experiment['dtree'] = [{'clf__min_samples_split': [2, 10, 30],
    ...
    for name in operation_steps.keys():
        # GridSearchCV
        gs = GridSearchCV(estimator=operation_steps[name],
